...
public class IncomeReportServiceImpl implements IncomeReportService {
	...
	public File generateIncomeReport(IncomeList incomelist) {
		...
		for (IncomeReportModel model : list) {
			filecontent.append(model.getBillid()).append(",").append(model.getRoomno()).append(",")
					.append(model.getGuestname()).append(",").append(model.getBilldate()).append(",")
					.append(model.getTotalbill()).append("\n");
			totalIncome += model.getTotalbill();
		}
		filecontent.append("\n");
		filecontent.append("Total_Income").append(",").append("Salaries_Paid").append(",").append("Profit")
				.append("\n");
